...
interface LayoutProps {
  ...
  userRole?: 'MEMBER' | 'CHAPTER_LEAD';
  onSignOut: () => void;
}

const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, userRole, onSignOut }) => {
  ...
  const tabs = [
    ...
    { id: 'new-referral', label: 'New Referral', icon: PlusSquare },
    { id: 'referral-status', label: 'Referral Status', icon: Activity },
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto scrollbar-hide min-h-0">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => {
      ...
      {/* Mobile Bottom Navigation - Simplified for core actions */}
      <nav className="md:hidden fixed bottom-6 left-6 right-6 glass rounded-3xl border border-white/40 flex justify-around items-center px-4 py-3 z-50 shadow-2xl pb-safe">
        {tabs.filter(t => ['dashboard', 'new-referral', 'meetings'].includes(t.id)).map((tab) => (
          <button
            key={tab.id}
          ...
          >
            <div className="relative">
              <tab.icon className={`w-5 h-5 ${activeTab === tab.id ? 'fill-indigo-600/10 stroke-[2.5px]' : 'stroke-[2px]'}`} />
            </div>
        ...
        <button
          ...
          className={`flex flex-col items-center flex-1 py-1 transition-all duration-300 ${
            activeTab === 'profile' ? 'text-indigo-600 scale-110' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          <User className={`w-5 h-5 ${activeTab === 'profile' ? 'fill-indigo-600/10 stroke-[2.5px]' : 'stroke-[2px]'}`} />
          <span className="text-[9px] mt-1 font-extrabold uppercase tracking-tighter">Me</span>
        </button>
      </nav>
    </div>
  );
