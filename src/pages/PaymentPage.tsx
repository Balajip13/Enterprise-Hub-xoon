...
const PaymentPage: React.FC<PaymentPageProps> = ({ onPaymentSuccess }) => {
  ...
  useEffect(() => {
    ...
    const fetchConfig = async () => {
      try {
        const res = await apiFetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/public/payment-config`);
  ...
  const handleDemoPayment = async () => {
    try {
      ...
      const response = await apiFetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/course/demo-purchase`, {
        method: 'POST',
  ...
  const handleRazorpayPayment = async () => {
    try {
      ...
      const response = await apiFetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/payments/create-order`, {
        method: 'POST',
        headers: { 
      ...
      const options = {
        ...
        handler: async function (res: any) {
          try {
            const verifyRes = await apiFetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/payments/verify`, {
               method: 'POST',
               headers: {
                 'Content-Type': 'application/json',
                 ...(token && { 'Authorization': `Bearer ${token}` })
